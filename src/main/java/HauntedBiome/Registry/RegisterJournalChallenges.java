package HauntedBiome.Registry;

import necesse.engine.journal.JournalChallenge;
import necesse.engine.journal.MultiJournalChallenge;
import necesse.engine.registries.JournalChallengeRegistry;
import necesse.inventory.lootTable.LootItemInterface;
import necesse.inventory.lootTable.LootTable;
import necesse.inventory.lootTable.lootItem.LootItem;

public class RegisterJournalChallenges 
{
    public void Register()
    {
        /* todo: Each reward is a part of a key. With all three parts of this key, turn them into a full key at a crafting station,
        then enter the secret void portal for a special boss fight.
         */

        // Surface Challenges
        int ACQUIRE_HAUNTED_SURFACE_TRINKET;
        int CRAFT_BLOODBERRY_JAM;

        // Caves Challenges
        int DESTROY_VOID_CRYSTALS;
        int DEFEAT_VOID_GATEWAY_ID;
        int ACQUIRE_HAUNTED_CAVES_TRINKET_ID;

        // Deep Caves Challenges
        int DEFEAT_DEEP_VOID_GATEWAY_ID;
        int ACQUIRE_DEEP_HAUNTED_CAVES_TRINKET_ID;
        int TOUCH_VOID_RIFT_WHILE_IMMUNE_ID;

        
    }
}
