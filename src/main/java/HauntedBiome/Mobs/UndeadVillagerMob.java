...
public class UndeadVillagerMob extends HostileItemAttackerMob 
{
  public static LootTable lootTable = new LootTable
  (
    new LootItemInterface[] 
    { 
        (LootItemInterface)ChanceLootItem.between(0.2F, "void_fragment", 1, 3), 
        (LootItemInterface)ChanceLootItem.between(0.2F, "coin", 2, 16).splitItems(2), 
        (LootItemInterface)new ChanceLootItem(0.05F, "healthpotion"), 
        (LootItemInterface)new ChanceLootItem(0.1F, "steak") 
    }
  );
  ...
  {
    super.init();
    this.ai = new BehaviourTreeAI((Mob)this, (AINode)new ConfusedItemAttackerPlayerChaserWandererAI(null, 512, getAIBattleAxe(), 40000));
  }
  ...
  public DeathMessageTable getDeathMessages() {
    return getDeathMessages("runebound", 3);
  }
  ...
  public void spawnDeathParticles(float knockbackX, float knockbackY) 
  {
    ...
    for (int i = 0; i < 4; i++)
      (getLevel()).entityManager.addParticle((Particle)new FleshParticle(getLevel(), gameSkin, GameRandom.globalRandom.nextInt(5), 8, 32, this.x, this.y, 10.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC); 
  }
  ...
  {
    if (!this.isAttacking && this.facingBuffer <= (float)getTime()) {
      super.setFacingDir(deltaX, deltaY);
      this.shouldResetFacingPos = true;
    } else if (this.isAttacking) {
      this.facingBuffer = (float)(getTime() + 500L);
